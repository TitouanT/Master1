...
extern
WINDOW * ecran_messages_droite_get( const ecran_t * const ecran )
{
  return( ecran->messages_droite ) ;
}
...
ecran_t * ecran_creer( )
{
  ...
  ecran->messages_droite = newwin( 3 , L/2, ecran->y_messages , L/2);
  box( ecran->messages_droite , ACS_VLINE , ACS_HLINE ) ;
...
void ecran_message_droite_display( ecran_t * const ecran , char * const mess )
{
 ...
 /* -------- */
  ...
  /* Affichage sur la fenetre */
  mvwprintw( ecran->messages_droite , 1 , 2 , mess_w ) ;

  wrefresh( ecran->messages_droite ) ;
}
